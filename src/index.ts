...
function flatMap<T, U>(items: readonly T[], map: (v: T) => U | readonly U[]): U[] {
    const results: U[] = [];
    items.forEach(item => {
...
function runInTerminal(text: string, filename: string, cmd: 'run' | 'dev') {
    ...
    const command = `cd ${JSON.stringify(casePath)} && npx vitest ${cmd} -t ${JSON.stringify(text)}`;
    terminal.sendText(command, true);
    terminal.show();
}

vscode.commands.registerCommand(RunVitestCommand.ID, (text: string, filename: string) => {
    runInTerminal(text, filename, 'run');
})

vscode.commands.registerCommand(DevVitestCommand.ID, (text: string, filename: string) => {
    runInTerminal(text, filename, 'dev');
})

const caseText = new Set(['it', 'describe'])

function tryGetVitestTestCase(callExpression: ts.CallExpression, file: ts.SourceFile): TextCase | undefined {
